...
// serverOf finds the spec for a kind, failing the test when it is missing.
...
// TestLegacyNamesStillApply keeps the pre-monorepo deployment working: the bare
// names are what the Aphlict-era compose files set.
func TestLegacyNamesStillApply(t *testing.T) {
	...
	if got := serverOf(t, cfg, ServerKindClient).Port; got != 9100 {
		t.Errorf("client port = %d, want 9100", got)
	}
	if got := serverOf(t, cfg, ServerKindAdmin).Port; got != 9101 {
		t.Errorf("admin port = %d, want 9101", got)
	}
	if got := serverOf(t, cfg, ServerKindClient).Listen; got != "10.0.0.1" {
		t.Errorf("listen = %q, want 10.0.0.1", got)
	}
}

// TestPrefixedNamesWinOverLegacyOnes pins the ordering the whole fallback scheme
// rests on. Both names set at once is what a half-migrated deployment looks like.
func TestPrefixedNamesWinOverLegacyOnes(t *testing.T) {
	...
	if got := serverOf(t, cfg, ServerKindClient).Port; got != 9200 {
		t.Errorf("client port = %d, want the prefixed 9200", got)
	}
	if got := serverOf(t, cfg, ServerKindAdmin).Port; got != 9201 {
		t.Errorf("admin port = %d, want the prefixed 9201", got)
	}
	if got := serverOf(t, cfg, ServerKindClient).Listen; got != "10.0.0.2" {
		t.Errorf("listen = %q, want the prefixed 10.0.0.2", got)
	}
}
...
func TestServerSpecAddr(t *testing.T) {
	tests := []struct {
		...
		// A bare port binds every interface, and an IPv6 literal has to keep its
		// brackets or the address will not parse.
...
// writeConfig drops an Aphlict-format config file into a temporary directory.
...
func TestLoadFromFile(t *testing.T) {
	...
	if cfg.Servers[0].Listen != DefaultListenAddr {
		t.Errorf("an entry without listen should default to %q, got %q",
			DefaultListenAddr, cfg.Servers[0].Listen)
	}
...
// TestLoadPrefersTheFileOverTheEnvironment checks that a config file replaces
// the environment's pair of servers rather than being merged into it, which
// would start ports the file never named.
func TestLoadPrefersTheFileOverTheEnvironment(t *testing.T) {
	...
	if got := serverOf(t, cfg, ServerKindClient).Port; got != 1234 {
		t.Errorf("client port = %d, want the file's 1234", got)
	}
}

// TestLegacyConfigFileNameStillApplies keeps APHLICT_CONFIG working, which is
// what the Aphlict-era deployments set.
func TestLegacyConfigFileNameStillApplies(t *testing.T) {
	path := writeConfig(t, `{"servers": [
		{"type": "client", "port": 1234},
		{"type": "admin", "port": 5678}
	]}`)
	...
	if got := serverOf(t, cfg, ServerKindAdmin).Port; got != 5678 {
		t.Errorf("admin port = %d, want the file's 5678", got)
	}
}
...
// TestLoadRejectsIncompleteConfigurations mirrors the check Phorge already makes:
// PhabricatorNotificationServersConfigType refuses a list without both kinds, so
// this reports the misconfiguration instead of silently not listening on a port.
...
// TestLoadAcceptsExtraServersOfEachKind leaves room for the Aphlict format's
// several-listeners case; main starts one listener per spec.
