...
| `schema-issues.json` | `GET /api/db/schema-issues` compares each selected replica with its partition master and flattens attribute/missing/surplus drift into `SchemaIssue` records; a consistent node reports an **empty array**, not null or an error. Canonical Phorge SchemaSpec comparison consumes `schema-diff` on the PHP side. |
| `setup-issues.json` | `GET /api/db/setup-issues` runs Phorge's own environment checks (version, InnoDB, the `meta_data` database, server variables); a healthy install reports an **empty array**, while an incomplete post-Ping query becomes a fatal `db.connection` issue. |
...
| `migrations-status.json` | `GET /api/db/migrations/status` reads `{namespace}_meta_data.patch_status` on the master selected for the `meta_data` partition and exposes its rows as `patch`; `initialized:false` means the database does not exist yet, while a post-Ping ledger failure is an explicit error. |
