...
| `migrations-status.json` | `GET /api/db/migrations/status` reads `{namespace}_meta_data.patch_status` on each master and exposes its rows as `patch`; `initialized:false` means the database does not exist yet, while a post-Ping ledger failure is an explicit error. |
