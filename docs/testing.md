...
**db-api 域的固件与 e2e 已落地。** db-api 的 Go 服务迁入的同时，契约固件与 e2e 脚本也一并补齐：`go/internal/dbapi/contract_test.go` 按 webhook / taskqueue 的形状写好，指向 `tests/contract/dbapi/`（七条只读路由 + 401 + 查询参数认证）与 `tests/contract/dbapi/unavailable/`（库连不上时 message 保持通用、body 不泄漏 SQL/库名/主机/端口）。
...
共享 runner 都只是薄 wrapper，真正的重放逻辑在 `go/internal/contracttest/`。它是 diff 迁入时从 render 的固件测试里抽出来的，抽出的理由不是省代码，而是**断言词汇必须在所有域之间保持一致**——各写一份 runner，后续域很快会开始用不同的方式描述自己的契约。它是普通包而非 `_test.go`，因为要被多个域的测试 import。conduit 因为每份固件都要配置不同的 proxy / limiter 状态而直接读取同一格式，例外写在 [`../tests/contract/conduit/README.md`](../tests/contract/conduit/README.md)。
...
`html*` 那组只有 render 域在用，但保留在共享词汇里——所有域要用同一套名字描述自己的契约。
...
**render 域**：正常渲染（python / go）、别名解析、大小写敏感的两条（`.R` / `.r`）、未知语言、空 source、CRLF、格式错误的请求体、未授权、查询参数认证、语言列表。

**diff 域**：hunk 头的三种计数形态、无尾换行的三种组合、identical 分支、normalize、prose 的三条、未授权、查询参数认证、格式错误的请求体。逐条对应见 [`../tests/contract/diff/README.md`](../tests/contract/diff/README.md)。

**notification 域**：admin 覆盖发消息、form-urlencoded 的 Content-Type、空 body、格式错误的 body、`/status/` 的扁平点号键、带 instance 的 `/status/` 与根探针；client 覆盖 `GET /`、实例路径、伪 Upgrade 请求与 `/healthz` 不被通配符吃掉。逐条对应见 [`../tests/contract/notification/README.md`](../tests/contract/notification/README.md)。

**search 域**：主目录覆盖写入、检索、索引生命周期、统计、后端列表、鉴权与格式错误；`unavailable/` 覆盖五个域级错误码，其中 `ERR_CHECK_FAILED` 有 `/exists` 与 `/sane` 两条路径。

**file-storage 域**：写入覆盖默认按优先级、指名引擎、零字节文件与未知引擎；读取覆盖裸字节、失败信封、缺参数与非法 handle；删除覆盖幂等、缺参数与非法 handle，另有引擎列表与鉴权。
...
**webhook 域**：主目录覆盖 `stats`、`hooks`、未授权与查询参数认证；`unavailable/` 覆盖数据库不可达时的 500 `ERR_INTERNAL`，且 body 不出现库名、主机、端口或 SQL。故障固件是本域**没有域级错误码**这个决定的反面守卫：既然没有码承载细节，message 就必须保持通用。

**taskqueue 域**：主目录覆盖入队、租约、统计、任务列表、单任务与未授权；`unavailable/` 覆盖 `stats` 与 `tasks` 在后端不可达时答 500 `ERR_INTERNAL`，且 body 不出现库名、主机、端口、SQL 或 `connection refused`。worker 域**没有固件**：它唯一的端点读进程内计数器、永不失败，一个「请求加期望应答」的固件对它无可断言，那条路径由 `go/internal/worker/http_test.go` 覆盖。

**conduit 域**：未授权、限流、缺 Conduit 方法，以及成功请求的透明代理；runner 用 `httptest` 启动假上游，避免依赖真实 Phorge。

**db-api 域**：主目录覆盖八条只读路由、未授权与查询参数认证；`unavailable/` 覆盖 `schema-diff`、`charset-info` 的 `ERR_DB_UNREACHABLE` 和 `servers` 的 in-band `fail`，并断言 body 不泄漏 SQL、库名、主机或端口。
...
所有脚本都对着**已经在跑**的实例执行，自己不启动也不清理任何东西：
...
# 或
TOKEN=dev-token make e2e     # 运行 Makefile 收录的全部脚本
```
...
`taskqueue.sh` 十五条场景，而它与 webhook 那份最大的不同是**它能跑通本域的主要工作**：队列可写（`enqueue`），所以脚本能入一个任务、把它租回来、complete 掉，再读计数往前走——`archivedCount` 至少 +1（用「单调增长」而非精确算术断言，因为可能有并发 worker 在归档）。这也是它会留痕的原因：那个测试任务落在归档表里，这是对的——队列是一份日志。它用的 task class 是 Phorge 自己的 `PhabricatorTestWorker`，免得游荡的 worker 把它当真活干。前提与 webhook 同样硬：**必须有一个可达的后端（`{namespace}_worker` 库或 Redis）**，没有本地回退——每个端点都碰存储；`/readyz` 不通时脚本直接退出，因为后面每条都会失败。**worker（`gorge-worker`，`/api/worker/stats`）不在这些脚本里**：它的唯一端点读进程内计数器，一个 e2e 脚本对它无非再验一次探针与鉴权，而那已被 `go/internal/worker/http_test.go` 覆盖。

render、diff、mailer、search、file-storage、webhook、taskqueue 与 db-api 八份都在 `TOKEN` 为空时跳过 401 那条并明确打印 SKIP，而不是静默略过。
...
覆盖率是一次运行的产物，不在手写文档里维护逐包百分比。当前精确结果以 `make cover` 或 Release / 手动触发的 Go Test Report artifact 为准；这样新增域、移动代码或升级工具链时，不会留下一个看似精确却已经失效的表格。

读覆盖率时仍要区分三类边界：

- `cmd/*` 的启动与真实监听主要由 e2e 覆盖，包内百分比通常很低；db-api 只有可独立测试的密码选择辅助函数例外。
- `contracttest` 的重放逻辑由各域测试调用，默认 `go test` 不会把调用方的覆盖计到被调用包；需要观察整条路径时使用 `-coverpkg`。
- webhook、taskqueue 的 MySQL / Redis 实现，以及 mailer、search 的真实下游往返，不能由内存 fake 证明。单元测试验证本服务发出的请求与 SQL 形状，真实系统语义由集成测试和第 4 节的 e2e 负责。

`httpx.RunAll` 是一个值得保留的正例：测试用 `:0` 端口起真实 listener 并发送真实信号，覆盖多监听器启动与并发关闭，而不需要启动完整二进制。

这也是为什么第 4 节的 e2e 不是「单元测试的补充」。对 Elasticsearch、Meilisearch、MySQL、Redis 与外部投递通道，它们是唯一能验证真实实现如何解释请求的层；`deploy/compose/demo/` 用于把服务和使用者选择的后端一起拉起。
