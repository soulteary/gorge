...
| 域层 | `internal/render`、`internal/diff`、`internal/notification`、`internal/mailer`、`internal/search` | 业务逻辑与路由 | 平台层 + 契约层 |
...
**新增域包时要同步往 `forbiddenPrefixes` 里加一行**，否则这个测试对新域是失效的——它照样通过，只是不再检查任何新东西。除 render 之外的四行都是各自迁入时手工补的，这是目前唯一需要人工维护的地方，改进建议见 [`findings.md`](findings.md) 第 8 条。

### 3.2 契约层：四个消费方的唯一真源

`internal/contracts` 一共 323 行（`render.go` 20 + `diff.go` 42 + `notification.go` 37 + `mailer.go` 74 + `search.go` 150），装的是过网络的数据结构，包注释写明了它的地位：
...
```
   internal/contracts/{render,diff,notification,mailer,search}.go
                                │
        ┌───────────────┬───────┴───────┬────────────────┐
        ▼               ▼               ▼                ▼
